    ...
    <section id="faq" className="py-20 lg:py-28 relative">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        <motion.div {...fadeUp()} className="text-center mb-14">
          <span className="text-muted-foreground text-xs font-semibold tracking-widest uppercase">FAQ</span>
          <h2 className="font-display text-3xl lg:text-4xl xl:text-5xl font-bold mt-3 mb-4 text-foreground">
            Common Questions
          </h2>
        </motion.div>

        <motion.div {...fadeUp(0.1)} className="glass-strong rounded-2xl p-5 sm:p-6 lg:p-8">
          <Accordion type="single" collapsible className="w-full">
            {faqs.map((faq, i) => (
              <AccordionItem key={i} value={`faq-${i}`} className="border-b border-border last:border-b-0">
                <AccordionTrigger className="text-left font-display font-medium hover:no-underline hover:text-foreground/70 transition-colors duration-200 py-4 text-foreground text-sm sm:text-base">
                  {faq.q}
                </AccordionTrigger>
